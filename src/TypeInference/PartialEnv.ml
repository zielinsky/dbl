...
let intersect_module_info ~check_schemes ~pp ~pos name info1 info2 =
  merge_opts info1 info2
    ~on_mismatch:(fun () ->
      Error.report (Error.or_pattern_modules_mismatch ~pos name))
    ~on_both:(fun info1 info2 ->
      ...
      (* Check values *)
      let vals1 =
        Name.Map.of_list (List.map (fun (n, _, sch) -> (n, sch)) info1.mi_vals)
      in
      let vals2 =
        Name.Map.of_list (List.map (fun (n, _, sch) -> (n, sch)) info2.mi_vals)
      in
      let check_val v_name sch1 sch2 =
        merge_opts sch1 sch2
          ~on_mismatch:(fun () ->
            Error.report (Error.or_pattern_vars_mismatch ~pos ~pp v_name))
...
let build_ren ~scope penv1 penv2 =
  ...
  let ren =
    Name.Map.fold
      (fun name info2 ren -> add_var_ren vars1 name info2.vi_var ren)
      penv2.val_map (T.Ren.empty ~scope)
  in
  StrMap.fold
    (fun mname minfo2 ren ->
      ...
      | Some minfo1 ->
          ...
          in
          List.fold_left
            (fun ren (name, var2, _) -> add_var_ren vars1 name var2 ren)
            ren minfo2.mi_vals
      | None -> ren)
...
let intersect ?check_schemes ~pp ~pos ~scope penv1 penv2 =
  ...
  if
    (not (T.TVar.Map.is_empty penv1.tvar_tab))
    || not (T.TVar.Map.is_empty penv2.tvar_tab)
  then Error.report (Error.or_pattern_binds_type_vars ~pos);
  ...
  let penv =
    {
      ...
      val_map =
        Name.Map.merge
          (intersect_val_info ~check_schemes ~pp ~pos)
          penv1.val_map penv2.val_map;
      module_map =
        StrMap.merge
          (intersect_module_info ~check_schemes ~pp ~pos)
          penv1.module_map penv2.module_map;
    }
  in
  (penv, ren)
